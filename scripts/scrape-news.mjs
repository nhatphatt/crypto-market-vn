...
function generateFeaturedSvg({ title, source, coins, dateLabel }) {
  ...
  const titleSvg = lines
    .map((line, i) => {
      ...
      return (
        '<text x="64" y="' +
        y +
        '" fill="#EAECEF" font-family="Segoe UI, system-ui, sans-serif" font-size="34" font-weight="700">' +
        escapeXml(line) +
        "</text>"
      );
    })
  ...
  // Build SVG with string concat (tránh lỗi parse template dài trên CI)
  const parts = [
    ...
    '<text x="108" y="66" fill="#EAECEF" font-family="Segoe UI, system-ui, sans-serif" font-size="18" font-weight="600">Crypto Market VN</text>',
    '<text x="108" y="86" fill="#707A8A" font-family="Segoe UI, system-ui, sans-serif" font-size="11">Crypto Market</text>',
    '<text x="64" y="128" fill="#929AA5" font-family="Segoe UI, system-ui, sans-serif" font-size="15">' +
      escapeXml(source) +
      " · " +
      escapeXml(dateLabel) +
      "</text>",
    ...
    '<text x="164" y="505" text-anchor="middle" fill="#181A20" font-family="Segoe UI, system-ui, sans-serif" font-size="14" font-weight="700">' +
      escapeXml(coinLabel) +
      "</text>",
    '<text x="64" y="560" fill="#707A8A" font-family="Segoe UI, system-ui, sans-serif" font-size="13">Tin thi truong · Tham khao</text>',
    "</svg>",
  ];
  return parts.join("\n");
}
